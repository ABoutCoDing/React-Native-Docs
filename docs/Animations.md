...
`LayoutAnimation`을 사용하면 다음 render/layout 주기에서 모든 뷰에 사용될 애니메이션 `create` 및 `update`를 전체적으로 구성 할 수 있습니다. 이는 특정 속성을 직접 애니메이션으로 표시하기 위해 특정 속성을 측정하거나 계산하지 않고 flexbox 레이아웃 업데이트를 수행하는 데 유용하며, 레이아웃 변경 사항이 상위 항목에 영향을 줄 수있는 경우 (예 : 부모의 크기를 늘리는 "더보기"확장)에 특히 유용합니다. 구성 요소를 모두 동기화하기 위해 구성 요소간에 명시적인 조정이 필요한 아래 행을 아래로 내립니다.
